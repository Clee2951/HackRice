...
export function PdfRead({ document = DEFAULT_DOCUMENT }: { document?: PdfDocument }) {
  ...
  return (
    <main className="relative min-h-svh overflow-hidden bg-transparent text-white">
      
      <div className="absolute inset-0 bg-black/45" aria-hidden="true" />

      <div className="relative z-10 grid min-h-svh grid-cols-[minmax(0,60vw)_minmax(360px,1fr)] gap-5 p-5">
        <section className="flex min-h-0 flex-col overflow-hidden rounded-2xl bg-amber-700/90 shadow-2xl">
          <header className="flex items-center justify-between border-b border-white/10 px-5 py-4">
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-slate-900">Now studying</p>
              <h1 className="mt-1 text-lg font-semibold">{document.title}</h1>
            </div>
            <span className="rounded-full border border-amber-400/40 px-3 py-1 text-xs text-amber-200">PDF</span>
          </header>
          <iframe title={document.title} src={document.src} className="min-h-0 flex-1 bg-white" />
        </section>

        <aside className="flex min-h-0 flex-col gap-4 overflow-y-auto">
          <section className="rounded-2xl bg-amber-700/85 p-5 shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-slate-900">Focus cycle</p>
          ...
          <section className="rounded-2xl bg-amber-700/85 p-5 shadow-xl">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-slate-900">Wellbeing check</p>
          ...
          <section className="flex min-h-64 flex-1 flex-col rounded-2xl bg-amber-700/85 p-5 shadow-xl">
            <div className="flex items-center gap-3">
              <div className="flex h-11 w-11 items-center justify-center rounded-full bg-amber-700 text-xs font-bold text-black shadow-[inset_0_0_0_2px_#991b1b]">AI</div>
              <div>
                <p className="text-xs uppercase tracking-[0.25em] text-slate-900">Study dealer</p>
            ...
            <form onSubmit={submitMessage} className="mt-5 flex gap-2">
              ...
              <button type="submit" className="rounded-lg bg-amber-300 px-4 py-2 text-sm font-semibold text-slate-900">Send</button>
            </form>
          </section>
        </aside>
      </div>
